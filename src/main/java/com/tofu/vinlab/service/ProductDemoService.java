...
public class ProductDemoService {
    ...
    public void demonstrateBuyableInterface(Buyable item) {
        ...
        logger.log(Level.INFO, "Доступно 5 единиц: {}", item.isAvailable(5));

        try {
            double total = item.buy(1);
            logger.log(Level.INFO, "Куплено через интерфейс. Стоимость: {} %n", total);
    ...
    public void demonstrateProduct(Product product) {
        ...
        logger.log(Level.INFO, "Товар: {}",  product);
        logger.log(Level.INFO, "Доступно 3 единицы: {}" , product.isAvailable(3));

        try {
            double total = product.buy(2);
            logger.log(Level.INFO, "Куплено 2 единицы. Общая стоимость: {}%n", total);
            logger.log(Level.INFO, "Остаток на складе: {}",  product.getStockQuantity());
        } catch (IllegalArgumentException e) {
            logger.log(Level.WARNING,"Ошибка покупки: {}", e.getMessage());
        }
        logger.info("%n");
    }
}
